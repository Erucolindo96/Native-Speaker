...
//                         <std::shared_ptr<GmmModel>>(l_thread.getModelPtr());
        QVariant model_ptr = QVariant::fromValue
                             <QObject*>(l_thread.getModelPtr().get());
...
void LearningController::updateDisplayedLearning()
{
  ...
  std::atomic_store(&act_displayed_learning_,
                    std::shared_ptr<GmmModel>(dynamic_cast<GmmModel*>(model_ptr.value<QObject*>()) ));
  actualizeProgressBar();
  actualizeComboBox();
}
...
//    connect(l_thread_combo_box_, SIGNAL(activated(int)),
//            this, SLOT(actualizeComboBox()));
  }
}
